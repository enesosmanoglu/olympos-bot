...
exports.run = async (bot, message = new Discord.Message(), params) => {
    let args = [];
    ...
    let kayıtsızRol = message.guild.roles.cache.find(
        e => e.name == "Peasant of Olympos"
    );
    if (!kayıtsızRol)
        return message.channel.send(
            new Discord.MessageEmbed()
    ...
    let taglıRol = message.guild.roles.cache.find(
        e => e.name == "Elite of Olympos"
    );
    if (!taglıRol)
        return message.channel.send(
            new Discord.MessageEmbed()
    ...
    let tagsızRol = message.guild.roles.cache.find(
        e => e.name == "Rebel of Olympos"
    );
    if (!tagsızRol)
        return message.channel.send(
            new Discord.MessageEmbed()
    ...
    if (isim.length > 12)
        return message.channel.send(
            new Discord.MessageEmbed()
                .setDescription(`İsim için harf limiti **12**'dir!`)
                .setColor(10038562)
                .setTimestamp()
        );

    if (!isim.match(/^[A-Za-zğĞüÜşŞıIiİöÖçÇ]+$/))
        return message.channel.send(
            new Discord.MessageEmbed()
                .setDescription(
    ...
    let cinsiyetRol = message.guild.roles.cache.find(e => e.name == "Apollo");
    if (!cinsiyetRol)
        return message.channel.send(
            new Discord.MessageEmbed()
    ...
    if (rMember.roles.cache.has(cinsiyetRol.id))
        return message.channel.send(
            new Discord.MessageEmbed()
                .setDescription("Bu kullanıcı zaten Apollo.")
                .setColor(10038562)
        );
    await rMember.roles.add(cinsiyetRol);

    if (taglı) await rMember.roles.add(taglıRol);
    else await rMember.roles.add(tagsızRol);
    ...
    rMember.isim = isim;
    rMember.yaş = yaş;
    rMember.cinsiyet = "erkek";
    await db.set(`users_${message.guild.id}.${rMember.user.id}`, rMember)

    console.log(`${rMember.user.tag} **${taglı ? "TAGLI" : "TAGSIZ"} ERKEK**  olarak kaydedildi. => ${rMember.displayName}`)

    let kayıtEmbed = new Discord.MessageEmbed()
        .setDescription(`${rMember} **${taglı ? "TAGLI" : "TAGSIZ"} ERKEK**  olarak kaydedildi.`)
        .setColor("00aeb1")
...
exports.help = {
    name: "erkek",
    description: "Erkekleri kayıt eder",
    usage: "erkek <kullanici> isim yaş"
};
